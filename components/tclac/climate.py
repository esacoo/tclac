...
SUPPORTED_FAN_MODES_OPTIONS = {
    "AUTO": ClimateMode.CLIMATE_FAN_AUTO,  # Immer verfügbar
...
SUPPORTED_SWING_MODES_OPTIONS = {
    "OFF": ClimateSwingMode.CLIMATE_SWING_OFF,  # Immer verfügbar
...
SUPPORTED_CLIMATE_MODES_OPTIONS = {
    "OFF": ClimateMode.CLIMATE_MODE_OFF,  # Immer verfügbar
    "AUTO": ClimateMode.CLIMATE_MODE_AUTO,  # Immer verfügbar
...
SUPPORTED_CLIMATE_PRESETS_OPTIONS = {
    "NONE": ClimatePreset.CLIMATE_PRESET_NONE, # Immer verfügbar
...
# Überprüfung der Interface-Konfiguration und Annahme von Standardwerten
    ...
    if CONF_VISUAL in config:
        ...
        if CONF_MIN_TEMPERATURE in visual_config:
            ...
            if min_temp < TCLAC_MIN_TEMPERATURE:
                raise cv.Invalid(f"Die angegebene minimale Interface-Temperatur von {min_temp} liegt unter der zulässigen {TCLAC_MIN_TEMPERATURE} für die Klimaanlage")
        else:
        ...
        if CONF_MAX_TEMPERATURE in visual_config:
            ...
            if max_temp > TCLAC_MAX_TEMPERATURE:
                raise cv.Invalid(f"Die angegebene maximale Interface-Temperatur von {max_temp} liegt über der zulässigen {TCLAC_MAX_TEMPERATURE} für die Klimaanlage")
        else:
        ...
        if CONF_TEMPERATURE_STEP in visual_config:
            ...
            if ((int)(temp_step * 2)) / 2 != temp_step:
                raise cv.Invalid(f"Der angegebene Temperaturschritt {temp_step} ist nicht korrekt, muss ein Vielfaches von 1 sein")
        else:
...
# Überprüfung der Komponenten-Konfiguration und Annahme von Standardwerten
...
# Registrierung der Ereignisse zum Ein- und Ausschalten des Klimaanlagen-Displays
...
# Registrierung der Ereignisse zum Ein- und Ausschalten des Klimaanlagen-Pieptons
@automation.register_action(
...
# Registrierung der Ereignisse zum Ein- und Ausschalten der Modul-Kommunikations-LEDs
@automation.register_action(
...
# Registrierung der Ereignisse zum Ein- und Ausschalten der erzwungenen Anwendung von Einstellungen
@automation.register_action(
...
# Registrierung des Ereignisses zum Einstellen der vertikalen Lamellenfixierung
@automation.register_action(
...
# Registrierung des Ereignisses zum Einstellen der horizontalen Lamellenfixierung
@automation.register_action(
...
# Registrierung des Ereignisses zum Einstellen der vertikalen Lamellenschwingung
@automation.register_action(
...
# Registrierung des Ereignisses zum Einstellen der horizontalen Lamellenschwingung
@automation.register_action(
...
# Hinzufügen der Konfiguration zum Code
def to_code(config):
